...
public class PowerPlantEntryParser implements CSVEntryParser<PowerPlant> {
    ...
    public PowerPlant parseEntry(String... data) {
        ...
        if (data.length > 7 && !data[7].isEmpty()){
            dismantleTime = Integer.parseInt(data[7]);
        }
...
//            }
        ...
        try {
            return createPowerPlant(name, pgt, energyProducer, powerGridNode, age, capacity, efficiency, dismantleTime);
        } catch (NullPointerException e) {
            logger.log(Level.SEVERE, "ERROR: Name: \"{0}\",Pgt: {1}, EnergyProducer{2}, Node:{3}, Age:{4}, Capacity: {5}, Efficiency: {6}, Dismantletime: {7}", new Object[]{name, pgt, energyProducer, powerGridNode, age, capacity, efficiency, dismantleTime});
            throw e;
        }
    }
    ...
    private PowerPlant createPowerPlant(String name, PowerGeneratingTechnology technology,
            ...
            PowerGridNode location, int age, double capacity, double efficiency, int dismantleTime) {
        ...
        if(dismantleTime < 1000){
            plant.setExpectedEndOfLife(dismantleTime);
        } else {
