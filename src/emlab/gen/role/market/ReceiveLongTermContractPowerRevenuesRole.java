 ...
 * 
 */
public class ReceiveLongTermContractPowerRevenuesRole extends AbstractEnergyProducerRole<EnergyProducer> implements Role<EnergyProducer> {

    public ReceiveLongTermContractPowerRevenuesRole(Schedule schedule) {
        super(schedule);
    }
   ...
   @Override
    public void act(EnergyProducer producer) {
        ...
        for (Segment segment : getReps().segments) {
            for (LongTermContract longTermContract : getReps().findLongTermContractsForEnergyProducerForSegmentActiveAtTime(
                ...
                getReps().createCashFlow(longTermContract.getTo(), longTermContract.getFrom(), updatedPrice,
        ...
        // for (PowerPlantDispatchPlan plan : getReps().powerPlantDispatchPlanRepository
        ...
        // double price = 0d;
        // for (ClearingPoint point : getReps().clearingPointRepositoryOld.findClearingPointsForSegmentAndTime(plan.getSegment(),
        // getCurrentTick())) {
        ...
        // getReps().nonTransactionalCreateRepository.createCashFlow(plan.getBiddingMarket(), plan.getBidder(), plan.getAcceptedAmount()
