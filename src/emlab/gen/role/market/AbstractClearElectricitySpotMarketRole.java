 ...
 *
 */
public abstract class AbstractClearElectricitySpotMarketRole<T extends EMLabModel> extends AbstractRole<T> {

    protected final double epsilon = 1e-3;

    public AbstractClearElectricitySpotMarketRole(Schedule schedule) {
        super(schedule);
    }
    ...
    CO2SecantSearch co2PriceSecantSearchUpdate(CO2SecantSearch co2SecantSearch, EMLabModel model,
     ...
     * @param marginalCostMap the marginal cost (including Co2 cost)
     */
    void determineCommitmentOfPowerPlantsOnTheBasisOfLongTermContracts(List<Segment> segments, boolean forecast) {
        ...
        for (EnergyProducer producer : getReps().energyProducers) {
            ...
            for (Segment segment : segments) {
                ...
                for (LongTermContract ltc : getReps().findLongTermContractsForEnergyProducerForSegmentActiveAtTime(producer,
                        segment, getCurrentTick())) {
                    contractedCapacityInSegment += ltc.getCapacity();
                }

                // for all power plants in the sorted marginal cost map
                for (PowerPlantDispatchPlan plan : getReps().findAllPowerPlantDispatchPlansForEnergyProducerForTimeAndSegment(segment, producer,
    ...
    Map<ElectricitySpotMarket, Double> determineActualDemandForSpotMarkets(Segment segment, Map<ElectricitySpotMarket, Double> demandGrowthMap) {
        ...
        for (ElectricitySpotMarket market : getReps().electricitySpotMarkets) {
            double baseLoad = getReps().returnSegmentBaseLoadBySegmentAndMarket(segment, market);
            double load;
            ...
            for (EnergyConsumer consumer : getReps().energyConsumers) {
                // for each active LTC
                for (LongTermContract ltc : getReps().findLongTermContractsForEnergyConsumerForSegmentForZoneActiveAtTime(
    ...
    double determineTotalEmissionsBasedOnPowerPlantDispatchPlan(boolean forecast, long clearingTick) {
        ...
        //int counter = 0;
        for (PowerPlantDispatchPlan plan : getReps().findAllPowerPlantDispatchPlansForTime(
    ...
    double determineTotalEmissionsBasedOnPowerPlantDispatchPlanForEnergyProducer(boolean forecast, long clearingTick,
        ...
        double totalEmissions = 0d;
        // int counter = 0;
        for (PowerPlantDispatchPlan plan : getReps().findAllAcceptedPowerPlantDispatchPlansForEnergyProducerForTime(producer, clearingTick, forecast)) {
            double operationalCapacity = plan.getCapacityLongTermContract() + plan.getAcceptedAmount();
            double emissionIntensity = plan.getPowerPlant().calculateEmissionIntensity();
            double hours = plan.getSegment().getLengthInHours();
            totalEmissions += operationalCapacity * emissionIntensity * hours;
     ...
     *
     * @param co2PriceStability the co2PriceStability so far
     * @param model the model for some of the parameters used in the
     * determination of stability.
     * @param government the government for some of the parameters used in the
     * determination of stability.
    ...
    CO2PriceStability determineStabilityOfCO2andElectricityPricesAndAdjustIfNecessary(CO2PriceStability co2PriceStability,
            EMLabModel model, Government government, boolean forecast,
            long clearingTick) {
    ...
    double findLastKnownPriceOnMarket(DecarbonizationMarket market) {
        Double average = calculateAverageMarketPriceBasedOnClearingPoints(getReps().findClearingPointsForMarketAndTime(market, getCurrentTick(), false));
